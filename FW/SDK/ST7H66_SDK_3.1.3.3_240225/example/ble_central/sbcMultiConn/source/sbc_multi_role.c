/**************************************************************************************************
    Filename:       sbc_multi_role.c
    Revised:
    Revision:

    Description:    This file contains the sbc multi role sample application
...
*/
// unit 1.25ms
#define DEFAULT_CONN_MIN_INTV                   128//40 
...
#define DEFAULT_ACTION_AFTER_LINK           ( GAPMULTI_CENTRAL_SDP )
...
static void multiRoleAppProcessGATTMsg( gattMsgEvent_t* pMsg )
{
    ...
    case ATT_WRITE_RSP:
    {
        ...
        pReq.sig = 0;
        pReq.cmd = 0;
        pReq.handle = 3;//multiRole_wrChar_handle ;
        pReq.len = 2;
...
static void multiRoleEstablishCB( uint8 status,uint16 connHandle,GAPMultiRole_State_t role,uint8 perIdx,uint8* addr )
{
    if( status == SUCCESS )
    {
        LOG("Establish success connHandle %d,role %d--->",connHandle,role);
        LOG( bdAddr2Str( addr ) );
        LOG("\n\n");
//        LOG("\nEstablish success connHandle %d,perIdx %d\n",connHandle,perIdx);

        if( role == Master_Role )
        {
            #if( MAX_CONNECTION_MASTER_NUM > 0)
            ///del the current node when the establishment is successful in sbc_multi_schedule.c
            ...
            if( multiGetSlaveConnList() != NULL )
            {
                ...
                scan_init_node_num =  sbcmc_multiRole_findInitScanNode();
...
static void multiRoleAPP_ScIn_Init(void)
{
    ...
    uint8 addr[B_ADDR_LEN]= {0x00,0x65,0x65,0x56,0x56,0x78};
...
static void multiRoleScanDoneCB( GAPMultiRolScanner_t* node )
{
    ...
    while( node )
    {
        if( ( multiDevInConfSlaveList( node->addr ) ) && \
                ( multi_devInLinkList( node->addr ) == FALSE) && \
                ( multiRole_find_node(MULTI_LINK_MODE,node->addr) == FALSE ))
        {
