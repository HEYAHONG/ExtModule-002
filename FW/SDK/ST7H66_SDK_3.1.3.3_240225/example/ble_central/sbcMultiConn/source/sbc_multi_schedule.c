...
///2022 08 08 prevent add too many init nodes, the number of current scan and init nodes is used as the limiting condition
...
uint8 multiRole_find_node(uint8 mode,uint8* data)
{
    ...
    case MULTI_LINK_MODE:
    {
        ...
        while(node )
        {
            if( osal_memcmp(node->peerDevAddr, data, B_ADDR_LEN) == TRUE)
            {
                ret = TRUE;
                break;
            }
...
/*******************************************************************************************************
    @ Description    :  multi role scheduler initilization , define in sbc_multi_schedule.h
 *******************************************************************************************************/
