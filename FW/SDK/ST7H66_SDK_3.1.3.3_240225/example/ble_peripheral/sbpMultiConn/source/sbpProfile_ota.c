                                            ...
                                            uint8* pValue, uint16 len, uint16 offset )
{
    ...
    if ( pAttr->type.len == ATT_BT_UUID_SIZE )
    {
        ...
        //LOG_DEBUG("[WT_ATT]connId %d :%04x: ",connHandle,uuid);
        ...
        case SIMPLEPROFILE_CHAR7_UUID:
            ...
            if ( status == SUCCESS )
            {
                ...
                simpleProfile_Notify(connHandle,SIMPLEPROFILE_CHAR6,len,pValue );
//          uint8 i;
//          if(len>20)
//          {
//              for(i=0;i<10;i++)
...
bStatus_t simpleProfile_Notify(uint16 connHandle, uint8 param, uint8 len, void* value )
{
    bStatus_t ret = SUCCESS;
    uint16 notfEnable = 0;
    uint16 notfConnId=INVALID_CONNHANDLE;
    ...
    case SIMPLEPROFILE_CHAR6:
        for (int i=0; i<GATT_MAX_NUM_CONN; i++)
        {
            ...
            {
                notfEnable = GATTServApp_ReadCharCfg( simpleProfileChar6Config[i].connHandle, simpleProfileChar6Config );
                notfConnId = i;
                break;
            }
        }
        ...
        else if ( notfEnable & GATT_CLIENT_CFG_NOTIFY )
        {
            ...
            simpleProfileChar6[ATT_GetCurrentMTUSize(connHandle)-4-1]=simpleProfileChar6[0];
            simpleProfileChar6[ATT_GetCurrentMTUSize(connHandle)-3-1]=simpleProfileChar6[1];
