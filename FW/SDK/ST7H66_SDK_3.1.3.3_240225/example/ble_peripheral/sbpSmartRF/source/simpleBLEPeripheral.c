...
//        LOG_DEBUG("[PHY] %02d %02d %d\n",allPhy,txPhy,status);
        ...
        #endif
        //[0x10 a1 a2 a3] :set smart rf tx
        else if(newValue[0]==0x10)
        {
            uint32 txIntv = newValue[1];
            uint32 txDura = newValue[2];
            uint8 dlen   = newValue[3];
            txDura = (txDura==0xff) ? 0xffffffff:txDura;

            if(txIntv==0)
            {
                uint8_t ret=phy_rf_stop_tx();
                LOG("[PPP TX] Stop ret %d st%d\n", ret,phy_rf_get_current_status());
            }
            else
            {
                LOG("[PPP TX] Intv%04d Dura%04d dlen%04d\n", txIntv,txDura,dlen);
                phy_tx_send_data( advertData, dlen,txIntv,txDura);
            }
        }
        else if(newValue[0]==0x11)
        {
            uint32 rxTimeOut   = newValue[1];
            uint8_t ret;

            if(rxTimeOut==0)
            {
                ret=phy_rf_stop_rx();
                LOG("[PPP RX] Stop ret %d st%d\n", ret,phy_rf_get_current_status());
            }
            else
            {
                ret=phy_rf_start_rx( rxTimeOut*1000);
                LOG("[PPP RX] rxTo%04d ret %d st%d\n", rxTimeOut,ret);
            }
        }
