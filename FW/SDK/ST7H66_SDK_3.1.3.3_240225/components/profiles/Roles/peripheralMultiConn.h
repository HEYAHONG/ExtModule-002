...
#define GAPROLE_PARAM_UPDATE_CONN_HANDLE 0x324
#define GAPROLE_CONNECTION_ACTIVE_NUM 0x325

#define GAPROLE_DISCONNECT_REASON  0x326

#define GAPROLE_PERIPHERAL_MAX_CONN              (MAX_NUM_LL_CONN)  // maxium allowed connection link
...
typedef enum
{
    ...
    GAPROLE_CONNECTED_TO_TERMINA,           //!< TERMINA ONE connection
...
*/
typedef void (*gapRolesStateNotify_t)(uint16 connHandle,gaprole_States_t newState );
...
extern bStatus_t GAPRole_GetParameter_MultiConn(uint16 connId, uint16 param, void* pValue );

extern uint8 GAPRole_Connect_Active_Num(void);
...
*/
extern bStatus_t GAPRole_TerminateConnection( uint16 connId);
